...
class acfe_pro_module_posts{
    ...
    function check_sync(){
        ...
        }elseif(acf_maybe_get_GET('post') && acf_maybe_get_GET('action2') === 'acfe_sync_item'){
    ...
    function check_duplicate(){
        ...
        // todo: manage bulk sync from db / php /json actions
        $ids = array();
    ...
    function admin_bulk_actions($actions){
        ...
        if($this->sync){
            ...
            $actions['acfe_sync_item'] = __('Sync changes', 'acf');
    ...
    function edit_columns($columns, $module){
        ...
        if(acf_get_instance('acfe_module_local')->is_php_enabled()){
            $columns['acfe-autosync-php'] = __('PHP', 'acfe');
        }
        
        if(acf_get_instance('acfe_module_local')->is_json_enabled()){
